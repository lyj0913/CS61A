...
def taxicab(a, b):
    ...
    """
    "*** YOUR CODE HERE ***"
    st=street(a)-street(b)
    ave=avenue(a)-avenue(b)
    return abs(st)+abs(ave)
...
def replace_leaf(t, old, new):
    ...
    if is_leaf(t):
        ...
        newbranches = [replace_leaf(b, old, new) for b in branches(t)]
...
def total_weight(m):
    ...
    if is_weight(m):
        ...
        assert is_mobile(m), "must get total weight oqf a mobile or a weight"
...
def balanced(m):
    ...
    if is_weight(m):
        ...
        lef,rig=end(left(m)),end(right(m))
        torqueleft=length(left(m))*total_weight(lef)
        torqueright=length(right(m))*total_weight(rig)
        return balanced(lef) and balanced(rig) and torqueleft==torqueright
